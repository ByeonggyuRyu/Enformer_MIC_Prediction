...
* correspondence: louisryu@kaist.ac.kr
"""
  ...
  def __init__(self,
    ...
    def conv_block(filters, width=1, w_init=None, name='conv_block', **kwargs):
      return Sequential(lambda: [
          ...
          gelu,
          tf.keras.layers.Conv1D(filters, width, **kwargs)
      ], name=name)

    stem = Sequential(lambda: [
        tf.keras.layers.Conv1D(channels, 7),
        pooling_module(pooling_type, pool_size=2),
        conv_block(channels, 7),
        pooling_module(pooling_type, pool_size=2),
        conv_block(channels, 3),
        pooling_module(pooling_type, pool_size=2),
        conv_block(channels, 3),
        pooling_module(pooling_type, pool_size=2),
        Residual(conv_block(channels, 1)),
        pooling_module(pooling_type, pool_size=2)
    ], name='stem')
    ...
    transformer = Sequential(lambda: [
        Sequential(lambda: [
            ...
            snt.LayerNorm(axis=-1,
                          ...
                          scale_init=snt.initializers.Ones()),
            tf.nn.relu,
            tf.keras.layers.Dense(channels)], name=f'transformer_block_{i}')
        for i in range(num_transformer_layers)], name='transformer')

    final_pointwise = Sequential(lambda: [
        pooling_module(pooling_type, pool_size=2),
        gelu,
        tf.keras.layers.Flatten()], name='final_pointwise')

    self._trunk = Sequential([stem,
                              transformer,
                              final_pointwise], name='trunk')
    trunk_name_scope.__exit__(None, None, None)

    final_dense = Sequential(lambda: [
        ...
        tf.keras.layers.Dense(11, activation='softmax')], name='final_dense')
